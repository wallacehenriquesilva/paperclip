...
import { Clock3, Cpu, FlaskConical, KeyRound, Puzzle, Settings, Shield, SlidersHorizontal, Terminal, UserRoundPen } from "lucide-react";
    ...
    <aside className="w-full h-full min-h-0 border-r border-border bg-background flex flex-col">
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide flex flex-col gap-4 px-3 py-2">
        <div className="flex flex-col gap-0.5">
          ...
          <SidebarNavItem to="/instance/settings/ai-auth" label="AI authentication" icon={KeyRound} end />
          <SidebarNavItem to="/instance/settings/terminal" label="Terminal" icon={Terminal} end />
          {(plugins ?? []).length > 0 ? (
